...
<head>
    ...
    <script src="{{ asset('js/paths_new.js') }}"></script>
    <script src="{{ asset('js/mapVisualization.js') }}" defer></script>
...
<body class="bg-gray-100">
    ...
    <div class="max-w-7xl mx-auto p-6">
        ...
        <div class="text-center mb-8">
            <h2 class="text-3xl font-bold text-gray-800 mb-4">Interactive Plot Booking System</h2>
            <p class="text-gray-600">Click on any plot to view details and book your preferred location</p>
        </div>

        Plot Information Display
        <!-- <div id="plotInfo" class="mb-6">
...
<script>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Initialize plot interaction and polygon data loading
        ...
        // Handle hover events with 3D effect
        function handleHover(event) {
            ...
            // Visual hover effect with enhanced 3D styling
            plot.classList.add('plot-hover');
            plot.style.stroke = '#007bff';
            plot.style.strokeWidth = '6';
            plot.style.filter = 'drop-shadow(8px 8px 15px rgba(0,0,0,0.4)) brightness(1.2)';
            plot.style.transform = 'scale(1.05) translate(-3px, -3px)';
            plot.style.zIndex = '1000';
            
            // Enhance the plot label if it exists
            const plotLabel = plot.parentNode.querySelector(`text[data-plot-label="${plotId}"]`);
            if (plotLabel) {
                plotLabel.style.filter = 'drop-shadow(4px 4px 8px rgba(0,0,0,0.7))';
                plotLabel.style.transform = 'scale(1.1) translate(-1px, -1px)';
            }
        }
        
        // Handle mouse out events - remove 3D effects
        function handleOut(event) {
            ...
            // Reset plot label styling
            const plotLabel = plot.parentNode.querySelector(`text[data-plot-label="${plotId}"]`);
            if (plotLabel) {
                plotLabel.style.filter = '';
                plotLabel.style.transform = '';
            }
        }
        ...
        // Load and process paths from the embedded data
        function loadAndProcessPaths() {
            // This function can be used to process SVG paths if needed
            console.log('Processing plot paths...');
        }
        
        // Create enhanced 3D shadow effect for plots
        function create3DShadow(plot) {
            ...
            // Create shadow layer - stays in original position
            const shadowPolygon = plot.cloneNode(true);
            shadowPolygon.setAttribute('id', `shadow_${plotId}`);
            shadowPolygon.setAttribute('class', 'plot-shadow-3d');
            
            // Style the shadow to match plot status
            const status = plot.dataset.status;
            if (status === 'booked') {
                shadowPolygon.setAttribute('fill', 'rgba(220,53,69,0.2)');
                shadowPolygon.setAttribute('stroke', 'rgba(220,53,69,0.3)');
            } else {
                shadowPolygon.setAttribute('fill', 'rgba(40,167,69,0.2)');
                shadowPolygon.setAttribute('stroke', 'rgba(40,167,69,0.3)');
            }
            
            shadowPolygon.setAttribute('stroke-width', '2');
            shadowPolygon.style.filter = 'blur(4px)';
            shadowPolygon.style.opacity = '0.4';
            ...
            setTimeout(() => {
                shadowPolygon.style.filter = 'blur(8px)';
                shadowPolygon.style.opacity = '0.6';
                shadowPolygon.style.transform = 'scale(1.15)';
            }, 50);
        }
        
        // Remove 3D shadow effect with animation
        function remove3DShadow(plot) {
            const plotId = plot.dataset.plot;
            const shadowElement = document.getElementById(`shadow_${plotId}`);
            
            if (shadowElement) {
                // Animate shadow removal
                shadowElement.style.filter = 'blur(4px)';
                shadowElement.style.opacity = '0.4';
                shadowElement.style.transform = 'scale(1.0)';
        ...
        // Dynamic plot generation function for easy scaling to hundreds of layouts
        function generatePlotsFromCoordinates(coordinatesArray, layoutName = 'Layout', startingPlotNumber = 1) {
            const plots = [];
            coordinatesArray.forEach((coords, index) => {
                const plotNumber = startingPlotNumber + index;
                plots.push({
                    id: plotNumber.toString(),
                    block: String.fromCharCode(65 + Math.floor(index / 20)), // A, B, C... every 20 plots
                    coordinates: coords,
                    price: 1000000 + (index * 50000), // Incremental pricing
                    area: 2000 + (index * 100), // Incremental area
                    status: Math.random() > 0.8 ? 'booked' : 'available', // Random booking status
        ...
        function submitBooking() {
            ...
            if (window.selectedPlot) {
                ...
                alert(`Booking request submitted for Plot ${plotId}!\n\nWe will contact you shortly at ${email} or ${phone}.`);
                
                // Mark plot as booked (simulation)
                window.selectedPlot.dataset.status = 'booked';
                window.selectedPlot.style.fill = 'rgba(200,200,200,0.3)';
                window.selectedPlot.style.stroke = 'rgba(255,0,0,0.8)';
